...
## Copied from the Monte Carlo review essay folder, not
## edited since
...
hist(mymeans1, breaks=20, xlab="Means drawn from N(12,23) with sample=100", prob=T, xlim=rangex)

dev.new()
mymeans2 <- replicate(10000, simmean( 1000, 13, 23 ))
hist(mymeans2, breaks=20, xlab = "Means drawn from N(13,23) with sample=1000", prob = T, xlim = rangex)
...
plotPDF <- function(a = NULL, b = NULL){
    ...
    plot(x, xpdf, type="l", ylab="Probability Density", ylim=c(0,1.2*(max(xpdf))), main="")
...
plotSimMeans <- function(mymeans, a, b){
    ...
    hist(mymeans, prob=T, ylim=c(0, 1.3*max(den1$y)), col="gray98", xlab=expression("Estimated means"), main="")
    ...
    legend("topleft", c("Theoretical Density (CLT)","Observed Density"), lty=c(1,2))
}
...
if(SAVEPLOTS == TRUE ){
    ...
    pdf(file=fileNameBase, width=6, height=6, onefile = FALSE, family="Times", pointsize=12, paper="special")
}
