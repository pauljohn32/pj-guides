...
y1 <- -5 +  4*surveyQuestion -1 * surveyQuestion^2 - 0.5 * surveyQuestion^3 + 6*x + 10 * e

dat <- data.frame(y1, surveyQuestion, surveyQuestionf = factor(surveyQuestion, labels = c("A", "B", "C", "D", "E")))
dat$surveyQuestionn <- as.numeric(dat$surveyQuestionf)

dat2  <-  X[dat$surveyQuestionn, ]
dat3 <- cbind(dat, dat2)

m1 <- lm(y1 ~ .L, dat3)
summary(m1)
plotSlopes(m1, plotx = ".L")

m2 <- lm(y1 ~ .L + .Q, dat3)
summary(m2)
X <- as.data.frame(X)
X$survey <- 1:5
X$m2fit <- predict(m2, newdata = X)
lines(X[ , ".L"], X$m2fit, col="red")

m3 <- lm(y1 ~ .L + .Q + .C, dat3)
summary(m3)
X$m3fit <- predict(m3, newdata = X)
lines(X[ , ".L"], X$m3fit, col="green", lwd=3)


colnames(dat3)[8] <- "quartic"
m4 <- lm(y1 ~ .L + .Q + .C + quartic, dat3)
summary(m4)
X$m4fit <- predict(m3, newdata = X)
lines(X[ , ".L"], X$m4fit, col="purple", lwd=3)


n1 <- lm(y1 ~ surveyQuestionn, dat = dat3)
summary(n1)

n2 <- lm(y1 ~ surveyQuestionn + I(surveyQuestionn^2), dat = dat3)
summary(n2)

n3 <- lm(y1 ~ surveyQuestionn + I(surveyQuestionn^2)+ I(surveyQuestionn^3), dat = dat3)
summary(n3)
