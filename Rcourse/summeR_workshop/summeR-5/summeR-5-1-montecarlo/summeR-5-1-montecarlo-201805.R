...
zpr <- dt(x, df =2 )
lines(zpr ~ x, col = "green")


wpr <- dt(x, df = 100 )
lines(wpr ~ x, col = "deeppink", lwd = 2)
...
x <- seq(0.00001, 20, by = 0.001)
shape <- 5
scale <- 1
xpr <- dgamma(x, shape = shape,  scale = scale)
plot(xpr ~ x, type = "l")
...
shape1 <- .9
shape2 <- 2.6
xpr <- dbeta(x, shape1 = shape1,  shape2 = shape2)
plot(xpr ~ x, type = "l")
...
hist(x[x > -10 & x < 10], breaks=50, prob = T)
...
###################################################
## ?rlogis
x <- rlogis(10000, location = 0, scale = 1/1.8)
hist(x, breaks = 50)
mean(x)
var(x)
sd(x)
...
xpr <- dlogis(x, 0, 1/1.8)
plot(xpr ~ x, type = "l")


ypr <- dnorm(x, 0, 1)
lines(ypr ~ x, col= "red", lwd = 3)
...
###################################################
N <- 2000
size <- c(10, 100, 1000, 10000, 20000, 50000, 750000)
ohone <- 0.01 ## a joke!
...
## more control
set.seed(123)
myMeanDiffs <- vapply(1:2000, oneSim, N = 1000,
