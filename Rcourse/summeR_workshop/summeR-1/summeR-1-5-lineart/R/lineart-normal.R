...
if (SAVEME == TRUE){
  pdf(file = "Normal-2013.pdf", height = 6.5, width = 6.5, onefile = F, paper = "special", pointsize = 10)

} else {
    arch <-  Sys.info()[['sysname']]
    if (arch == "Windows") {
        windows(height = 6.5, width = 6.5, pointsize = 9)
    } else {
...
shadedArea <- round(pnorm(mu + 1.96 * sigma, mean = mu,
                          sd = sigma, lower.tail = F), 4)
...
text( mu + 1.96*sigma,1.15*ss,
     bquote(paste(.(round(criticalValue,2)), phantom(1) == mu+1.96 * sigma,sep = "")),
     pos = 2 )
