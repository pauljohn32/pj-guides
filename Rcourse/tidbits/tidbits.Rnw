\batchmode
\makeatletter
\def\input@path{{/home/pauljohn/SVN/SVN-guides/Rcourse/tidbits//}}
\makeatother
...
\item Every time you run
\end{itemize}
<<eval=F>>=
library(some-package-name)
@
\begin{itemize}
\item You should also run
\end{itemize}
<<eval=F>>=
...
\end{itemize}
\item R package ``sos'' offers search tools for finding material in CRAN

\begin{itemize}
\item Install that, then run 
...
\lyxframeend{}\lyxframe{{[}containsverbatim{]}debug package}
\begin{itemize}
