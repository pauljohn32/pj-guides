...
compileOne(){
	...
	if [[ -e "$filename" ]]; then
		...
		## last minute addon: run this to delete chunks with "excludme" in name.
		echo "base $bn"
...
## run this manually, can't figure how to get in script
perl -0pi.bak -e 's/\d*: excludeme(.*?)code chunk number //sg' sem-4-2-mediation.R
