...
# TESTFILE="~/semexample/Mplus/Ex-01-EFA/efa-01.org"
# echo $TESTFILE
...
$EMACSCMD -nw --batch --directory "$SRC_DIR" --eval "
          (progn
            ...
            (crmda/insert-crmda-template \"$SETUP_DIR\")
            (org-babel-tangle)
            (rename-output-section)
            (crmda/auto-insert-output-section)
            (save-buffer)
            (kill-buffer)
            (execute-shell-mplus-block \"$1\")
            (find-file (expand-file-name \"$1\"))
            (crmda/auto-insert-results-section)
            (save-buffer)
            (kill-buffer)
            (find-file (expand-file-name \"$1\"))
...
usage ()
{
    ...
    echo "    -c        {c}hange the name of the Output section to \"Run the Model\""
    echo "    -e        {e}xport the FILE to html"
    echo "    -M        execute the {m}plus source code block in the FILE"
    echo "    -m        export the FILE to markdown"
