...
exampleClasses <- c("GameTheory"="Game Theory",
                    "Networks"="Networks",
...
tdir.subdirs <- list.dirs(tdir,  recursive = TRUE)
## Drop subdirs that don't have any png
tdir.subdirs <-
    tdir.subdirs[vapply(tdir.subdirs,
                        function(x) length(list.files(x, pattern = "png$")) > 0,
                        logical(1))]
...
for (j in dirs) {
    ...
    for(i in fnsf) {
        ...
        RUN2 <- paste("convert -quality 100 -density 600 -flatten -strip -geometry 300x300",
                      fn.pdf, fn.png)
        system(RUN2)
        ...
        if (!(fn.png %in% names(md5sums)) || fn.png.md5 != md5sums[fn.png]) {
            fn.png.fullpath <- file.path(tdir.full, j, fn.png)
...
unlink(file.path(topdir, "README.md"))
...
fn.png <- list.files(path = "tex", pattern = "png$", recursive = TRUE,
                     full.names = TRUE)
names(fn.png) <- gsub(".png", "", fn.png)
fn.tex <- list.files(path = "tex", pattern = "tex$", recursive = TRUE,
                     full.names = TRUE)
## Remove the setupXX.tex files from the list
...
k <- 1
for (i in names(fn.png)) {

    texfn <- fn.tex[i]
    pathdirs <- unlist(strsplit(texfn, "/"))
    parentdir <- pathdirs[(length(pathdirs) - 1)]
    ...
    resline <- paste0("<a href=\"", texfn, "\">",
                      "<img src=\"", pngfn, "\" title=\"Open TeX Source\"",
                      ">", "</a>",
